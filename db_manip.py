...
def get_user_msgs(username: str = 'whetan', number: int = 10):
    '''
    return a list of stripped ethan messages
    '''
    conn = sqlite3.connect(DB)
    c = conn.cursor()
...
def vocab_mapping(messages: list[str]):
    '''
    return map of word id number to word for one hot encoding
    '''
    word_map = {}
    i = 0
    for msg in messages:
...
if __name__ == "__main__":
    ethan_msgs = get_user_msgs('whetan', 10)
    ethan_word_map = vocab_mapping(ethan_msgs)

    print(list(ethan_word_map.keys())[-1])
